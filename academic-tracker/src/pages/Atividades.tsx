...
export default function Atividades() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!novaAtividade.title || !novaAtividade.dueDate || !novaAtividade.grupoId) {
    ...
    setNovaAtividade({ title: "", description: "", dueDate: null, grupoId: null });
          ...
          <ul>
            {atividades.map((a) => (
              ...
              >
                ...
                📅 Entrega:{" "}
                {a.dueDate ? new Date(a.dueDate).toLocaleDateString() : "-"} | Grupo:{" "}
                {a.grupoId}
                <div style={{ marginTop: "0.5rem", display: "flex", gap: "0.5rem" }}>
                  <button onClick={() => handleEditar(a)}>✏️ Editar</button>
                  <button onClick={() => handleExcluir(a.id)}>🗑️ Excluir</button>
                </div>
              </li>
            ))}
          </ul>
